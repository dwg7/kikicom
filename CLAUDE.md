...
### 受信断の診断パターン:「候補は来るが有効化しない」= RF経路の物理障害(2026-09-23)

2026-09-23未明〜07:38、有効メッセージがほぼゼロ(直前ログは03:58が最後、以後
07:38の対処まで3.5時間超の空白)になる受信断が発生。切り分けの決め手になった
指標の組み合わせ:

| 指標 | 断中 | 平常時 |
|---|---|---|
| samples_dropped/lost(USB/SDR健全性) | 0/0(正常) | 0/0 |
| ノイズフロア | -42.0dBFS(平常と同じ) | 約-42dBFS |
| プリアンブル候補("modes"、生信号量) | 263,531件/分(平常並み) | 同水準 |
| **有効メッセージ("accepted")** | **0〜1件/分** | **数百件/分** |
| journalctl(サービスエラー・再起動) | なし | — |

**「生信号量・ノイズフロア・USB健全性は平常通りなのに、有効メッセージだけが
ほぼゼロ」という組み合わせは、ソフトウェア(サービスダウン・USB切断)でも
純粋なゲイン不足でもなく、アンテナ〜受信機間のRF経路そのものの物理障害
(給電点の接触不良・検波前の信号劣化)を示す。** 原因はカーテンがアンテナの
ワイヤを押して接触が悪くなったことと推定(ユーザー推定、未確定)。07:38に
物理的に調整して復旧、直後に有効メッセージ277件/分(過去の良好時の基準
=毎分約308件、9/21ゲイン40.2dB試験)とほぼ同水準まで即座に回復し、
仰角1.6°・2.5°の低仰角機も正常に捕捉できることを確認——「近距離の強い
信号だけ」ではない全域での回復と判断。ただし断の正確な開始時刻・原因は
未確定。同じ現象が**カーテンに触れていない状況で再発すれば、コネクタ自体の
劣化を疑う根拠になる**(触った時だけ起きるなら設置の脆さ、触らずに起きる
なら機材側、という切り分け)。履歴は`documents/antenna-placements.tsv`。

**気になる副産物:** 復旧後の9/23はピーク55機/時(15時台)と、9/22のピーク
32機/時を大きく上回った。曜日差(祝日→平日)による純粋な交通量増加の
可能性が高いが、「断になる前から接触不良が緩やかに受信効率を落として
いたのでは」という仮説も否定できない。基準線が増えたら検証する
...
### 20cm上げた効果の検証(2026-09-22、`~/kikicom-data/coverage/`実測)

旧位置(低い、9/21 17:30〜9/22 07:44・全期間)vs 新位置(20cm上げ・ガラス
中央、9/22 09:22〜。布団干しによる遮蔽期間 11:22〜16:00頃は除外。撤去時刻は
厳密には記録していないユーザー申告の目安)で仰角別の受信率を比較:

| 帯域 | 旧位置(低い) | 新位置(高い・枠から離す・布団期間除外) |
|---|---|---|
| 仰角4°未満 | 11.9% (8/67) | 21.3% (68/319) |
| **仰角2°未満** | **0.0% (0/46)** | **7.7% (15/195)** |
| 全仰角 | 24.5% (24/98) | 24.1% (87/361) |

**「低い仰角が勝負」という仮説通り、上げたことで仰角2°未満(最も苦しい帯域)が
0%から受信できるようになった。** 全仰角では差がほぼ消える一方、低仰角ほど
改善幅が大きく、仮説と整合的。同じ時間帯(17:38〜22:06、旧日/新日)に絞った
小サンプル版でも同じ傾向(4°未満14.0%→26.1%、2°未満0%→6.6%)で、結論は
サンプルサイズに対して頑健。

**重要な副産物:「上げる」より「金属枠から離す」の方が支配的要因だった。**
07:44に一度上げた直後(縦のアルミ枠に1〜2cm接近する設置、`window-high-frame`)は
むしろ悪化した(仰角4°未満で11.9%→5.3%、`documents/antenna-placements.tsv`参照)。
09:22にガラス中央(枠から20〜30cm以上)へ動かしてから改善が出ている。今後の
設置調整は「高さ」より「導体からの距離」を優先して考えること。

### 受信可能な方角の正確な理解(2026-09-22、2日分111機の方位・高度・距離を集計して検証)

- **新千歳(CTS)発着**(方位およそ135〜195°、南南東〜南)が交通の大半。低空から巡航高度(35,000ft超)まで
- **丘珠発着・近傍周回**(距離0〜35km、方位は全方位)。近いので方位を問わず取れる。函館等への巡行を含む
- **「カムチャツカ巡航」は独立した受信方向ではない。** AAL280・QTR8048・KAF001等の高高度国際便を検証した
  結果、受信できた瞬間の方位はCTS線と同じ135〜195°だった。真に北東方位(0〜90°)から受信できた例は
  2日間で一度もない。**これらの便は進行方向(トラック)こそ北東(カムチャツカ・北極圏方面)だが、
  受信できているのは「まだ南側にいる間」だけ**。北東へ抜けて本当に北東方位に入る頃には距離が
  離れすぎて受信範囲外になる。→「CTS線と同じ南の窓を、より遠く・より高く通過する交通」であり、
  CTS線の信号に対して分離可能な背景として同じ窓に入ってくる、と理解するのが正確
- **稚内線(まれ)**もANA574で確認、方位87〜196°・距離9〜80kmで、基本的に同じ南側の窓の中
- **結論:実質的な受信窓は1本(方位およそ130〜200°の南の扇)+丘珠近傍の全方位(近距離限定)。**
  真の北〜東の四分儀(方位0〜90°)は完全な死角のまま
- **この死角(0〜90°)は、窓の左側(東〜北東方向)に近接する高層マンションで説明できると考えられ、
  ユーザーの判断で「与条件」として受け入れる。** アンテナ位置の調整でこの方向を改善しようとする
  試みは行わない(今後の改善努力は南側の窓の質の向上に集中する)
- **地理院地図で北上の建物配置を目視確認済み(2026-09-22)。** 北東方向に目立って大きな建物があり、
  東〜南南東方向は開けている——電波だけから逆算した「窓の視界」と、実際の建物配置がほぼ一致した。
  座標・地図画像そのものはリポジトリに含めない(住所の特定につながるため)

### 一般航空(GA)の識別:災害応用のための分類精度

災害応用を見据えると、「未確認機として警戒する」より**「レジャー機・一般航空機として正しく識別できる」**
ことが重要、という方針(2026-09-22)。`review-aircraft.py`は、adsbdbに登録がない日本ブロック機のうち、
区分A1/A2(軽量機)かつVFR(スコーク1200)の実績がある機体を、「自衛隊・官公庁の候補」から
「一般航空の可能性が高い」区分に分離する(実例:JA42XX、コールサイン未設定と見られる軽航空機、
高度2000ft一定・速度70kt台で通過)。この変更後、警察庁のヘリ(JA04HP、AW139)は引き続き正しく
「公用機の候補」に分類されることを確認済み。

### 注目機:航空測量

ユーザーの主要な関心は**航空測量**で、その機体は重要(C208 などは航空測量機の可能性が高い)。
...
**2026-09-23、ヘリコプター型でも初のヒット2件**(固定翼のJA13AJだけでなく回転翼も
watchlistが正しく拾えることを確認):**JA118D**(EC135 P2+、中日本航空、12:34–14:05)、
**JA6912**(Bell 430、朝日航洋、15:24)。中日本航空・朝日航洋はどちらも航空測量・
レーザ測量で知られる運航者で、機種・所有者名のキーワード一致という設計が狙い通り機能した。
...
- [x] 20cm上げた効果の検証(同じ時間帯17:38〜22:06で比較。仰角2°未満 0%→6.6%、
  4°未満 14.0%→26.1%。ただし枠に近すぎると逆効果になることも判明。§3「20cm上げた効果の検証」)
