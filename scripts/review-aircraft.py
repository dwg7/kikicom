...
def review(day):
    ...
    sections = {
        ...
        "日本のブロックで登録不明・一般航空の可能性が高い(区分A1/A2・VFR実績あり)": [],
        "ヘリコプター(区分A7)": [],
        "Mode Sのみ(ADS-Bの位置なし)": [],
        "航空会社の便名形式でないコールサイン": [],
    ...
    for a in ac.values():
        ...
        # 例:2026-09-22 JA42XX(8571a9、高度約2000ft一定・速度70kt台・スコーク1200を送った実績あり)。
        # コールサイン未設定(プレースホルダー)の一般航空機と見られ、区分A1/A2でVFRスコークの実績が
        # あれば、自衛隊・官公庁の候補から分離する(災害応用では「レジャー機と識別できる」ことが
        # 「未確認機として警戒する」より重要)。区分不明(A0)や情報が薄い機体は、安全側に倒して
        # 引き続き要確認に残す
        likely_ga = bool(a["cats"] & {"A1", "A2"}) and "1200" in a["squawks"]
        if jp(a["hex"]) and db is None and not airline:
            if likely_ga:
                sections["日本のブロックで登録不明・一般航空の可能性が高い(区分A1/A2・VFR実績あり)"].append(a)
            else:
                sections["日本のブロックで登録不明・要確認(自衛隊・官公庁・新規登録の候補)"].append(a)
        if "A7" in a["cats"]:
